...
bool XModem::read_block_buffered(struct packet *p, byte *buffer) {
  ...
  //Serial.print("Received XMODEM block: " + String(b_pos) + " bytes\n");
  //hexdump(buffer, b_pos);
  b_pos = 0;
  for(size_t i = 0; i < _id_bytes; ++i) {
    p->id[i] = buffer[b_pos++];
  ...
  for(size_t i = 0; i < _chksum_bytes; ++i) {
    if(p->chksum[i] != buffer[b_pos++]) { 
      // checkum fail
      Serial.println("Checksum fail: expected ");
      for(size_t j = 0; j < _chksum_bytes; ++j) {
        Serial.print(p->chksum[j], HEX);
        Serial.print(" ");
      }
      Serial.print(" got ");
      for(size_t j = 0; j < _chksum_bytes; ++j  ) {
        Serial.print(buffer[b_pos - _chksum_bytes + j], HEX);
        Serial.print(" ");
      }
      Serial.println("Nonmatching p->chksum[i] != buffer[b_pos++] is " + String(p->chksum[i], HEX) + " != " + String(buffer[b_pos-1], HEX));
      Serial.println();
      return false;
    }
  }
...
void XModem::crc_16_chksum(byte *data, size_t dataSize, byte *chksum) {
  ...
  *crc = ((*crc & 0xFF00) >> 8) | ((*crc & 0x00FF) << 8); //swap byte order for big endian
}
